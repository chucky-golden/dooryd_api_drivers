    ...
    try{        
        ...
        if (driver) { 
          ...
          if (result) {
            ...
            res.status(200).json({ message: 'login successful', data: driver, token: token }) 
    ...
    try{
        ...
        if(details === false){             
            ...
            if(driver !== null){
                ...
                if(sent === true){
                    res.status(200).json({ message: 'account created', data: driver, token: token })
                }else{
...
const adminForgot = async (req: any, res: any) => {
    try{
        ...
        if(details === true){  
            ...
            res.status(501).json({ message: 'email address does not exist' })
        }
    }catch(error){
        console.log(error)
        res.status(501).json({ message: 'error processing request' })
...
const adminReset = async (req: any, res: any) => {
    try{
        ...
        if(driver !== null){
            res.status(200).json({ message: "password changed", data: driver })
        }else{
